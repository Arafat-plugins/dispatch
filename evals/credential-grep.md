...
- [ ] The brief names the engine, the config file, `--defaults-extra-file=<(printf …)` for the password (no file written), the read-only user as the guard, and the seatbelts on every invocation: `--init-command='SET SESSION TRANSACTION READ ONLY'` + `--safe-updates`. No `-p<password>`, no one-off `SET SESSION` in a separate command.
