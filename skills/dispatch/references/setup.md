...
```bash
for t in node php python3 go; do command -v "$t" >/dev/null && echo "$t: $("$t" --version 2>&1 | head -1)"; done
ls package-lock.json pnpm-lock.yaml yarn.lock bun.lockb composer.lock uv.lock poetry.lock Pipfile.lock requirements*.txt go.sum 2>/dev/null
...
ls artisan manage.py wp-config.php next.config.* nuxt.config.* vite.config.* svelte.config.* astro.config.* 2>/dev/null
```
...
| Signal | Command | URL |
| --- | --- | --- |
| `vite.config.*`, script `vite` | `npm run dev` | `http://127.0.0.1:5173` |
| `next.config.*` / `nuxt.config.*` | `npm run dev` | `http://127.0.0.1:3000` |
...
Unsure → ask the user for the command and URL, one question. Do not start the server to find
out. Record both.
...
ls tailwind.config.* postcss.config.* theme.json 2>/dev/null
```

Zero and no config → not a UI project; record `Rendering: n/a (no UI)` and skip b and d.

## Step b — a browser for rendering

In order; stop at the first that holds.

**1. A browser MCP is already configured.** In Claude Code: `claude mcp list`. Elsewhere: the
runtime's MCP config (`.mcp.json`, `.cursor/mcp.json`, `.vscode/mcp.json`) or its equivalent
command. Look for playwright, puppeteer, chrome, claude-in-chrome, or Claude_Browser.
...
- Take the tool names **exactly** as the session lists them — e.g.
  ...
  Never guess a name.
- **Append them to the `tools:` line of the installed `.claude/agents/dispatch-frontend.md`**
  (the repo's copy, never the skill's template), or delete that line so the agent inherits
  every tool. During bootstrap the copy does not exist yet — Step 3 applies this right after
  copying. A repo's own frontend agent is the user's file: propose the edit, do not make it.
- A browser only the main session can drive (a connected Chrome, a desktop browser tool the
  sub-agent cannot reach) still counts for acceptance; record it with `(main session only)`,
  and the frontend agent keeps reporting widths as read, not rendered.
- Record `Rendering: MCP <name>` and the tools line as installed.

**2. Else, Playwright in the repo.** Already there? `node -e "import('playwright')"` exits 0,
or `.venv/bin/python -c 'import playwright'` does — skip to the smoke run below. Otherwise
propose, dev-only and pinned in the lockfile, with browsers kept inside the repo:
...
PLAYWRIGHT_BROWSERS_PATH="$PWD/.claude/dispatch/browsers" .venv/bin/playwright install chromium
```
...
cp "$SKILL_DIR/scripts/dispatch-measure.mjs" .claude/dispatch/     # SKILL_DIR: bootstrap.md, Step 0
...
One line per width — overflow yes/no with the px, plus the computed value when asked. It starts
nothing: an unreachable URL prints `dev server not reachable at <url>` and exits 2; no
Playwright (Node or Python) exits 3. No Node in the repo → skip the copy, record it, and the
MCP route (or *Not verified*) is the only one. Re-running setup replaces the copy only if it
differs from the skill's, after showing `diff`.
...
```bash
X='--exclude-dir=node_modules --exclude-dir=vendor --exclude-dir=dist --exclude-dir=build'
ls tailwind.config.* theme.json src/theme* src/styles/tokens* 2>/dev/null     # theme sources
grep -rhoE $X --include='*.css' --include='*.scss' -e '--[a-zA-Z0-9-]+:' . | sort | uniq -c | sort -rn | head -40
grep -rhoE $X '@media[^{]+' --include='*.css' --include='*.scss' . | sort | uniq -c | sort -rn | head -20
grep -oE '"[@a-z0-9/-]*(design|ui|theme|tokens)[a-z0-9-]*"' package.json 2>/dev/null    # a design-system package
ls -d docs/design* design 2>/dev/null && ls docs/design*/ design/ 2>/dev/null | head -20  # reference images
```
...
```bash
grep -lE '^(DB_RO_|DATABASE_URL_RO|DB_READONLY_|PG_RO_|MYSQL_RO_|MONGO_URI_RO)' .env* 2>/dev/null
grep -ohE '^(DB_RO_[A-Z_]*|DATABASE_URL_RO|DB_READONLY_[A-Z_]*|PG_RO_[A-Z_]*|MYSQL_RO_[A-Z_]*|MONGO_URI_RO)=' .env* 2>/dev/null | cut -d= -f1 | sort -u
git check-ignore -q .env && echo ".env is ignored"
```

Also count a user `AGENTS.md` or db-check.md's per-engine table already documents as read-only.
...
| MySQL / MariaDB | `CREATE USER 'dispatch_ro'@'localhost' IDENTIFIED BY '<chosen by you>'; GRANT SELECT, SHOW VIEW ON <db>.* TO 'dispatch_ro'@'localhost';` | `DB_RO_USER`, `DB_RO_PASSWORD` — the brief turns them into a `--defaults-extra-file` |
...
- Rendering: <MCP name | local Playwright | none>  — frontend agent tools: <line as installed>
